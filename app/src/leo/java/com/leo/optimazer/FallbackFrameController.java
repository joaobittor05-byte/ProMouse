...
/** Frame Repeat usando somente o backend shell do Shizuku, sem UserService. */
final class FallbackFrameController {
    ...
    private FallbackFrameController() {}
    ...
    static synchronized String apply(String packageName, int requestedFps) {
        ...
        String peak = PrivilegedShell.runAllowFailure("settings put system peak_refresh_rate " + target);
        String min = PrivilegedShell.runAllowFailure("settings put system min_refresh_rate " + target);
        ...
        boolean verified = verifySettings(match.refresh);
        return "FRAME_REPEAT_OK"
                ...
                + " duplicates=" + Math.max(0, match.multiple - 1)
                + " cadence=" + cadenceLabel(match.multiple, match.integer)
                + " strategy=DISPLAY_BUFFER_HOLD"
                + " queue=0"
                + " backend=SHIZUKU_SHELL_FALLBACK"
                + " verified=" + verified
    ...
    static synchronized String status(String packageName) {
        ...
        return "FRAME_REPEAT_STATUS active=true"
                ...
                + " duplicates=" + Math.max(0, activeMultiple - 1)
                + " cadence=" + cadenceLabel(activeMultiple, true)
                + " strategy=DISPLAY_BUFFER_HOLD"
                + " queue=0"
                + " backend=SHIZUKU_SHELL_FALLBACK"
                + " verified=" + verified;
    ...
    static synchronized String reset(String packageName) {
        ...
        PrivilegedShell.runAllowFailure("dumpsys SurfaceFlinger --timestats -disable");
    ...
    private static FpsSample samplePackageFps(String packageName) {
        String start = PrivilegedShell.runAllowFailure("dumpsys SurfaceFlinger --timestats -clear -enable");
        ...
        String dump = PrivilegedShell.runAllowFailure("dumpsys SurfaceFlinger --timestats -dump");
        PrivilegedShell.runAllowFailure("dumpsys SurfaceFlinger --timestats -disable");
    ...
    private static List<Float> detectSupportedRates() {
        ...
        String dump = PrivilegedShell.runAllowFailure("dumpsys display");
    ...
    private static Match chooseMatch(int fps, List<Float> rates) {
        ...
        return new Match(max, Math.max(1, Math.round(max / Math.max(1, fps))), false, 1f);
    }
    ...
    private static String restoreSetting(String key, String value) {
        if (value == null || value.isEmpty() || "null".equalsIgnoreCase(value)) {
            return PrivilegedShell.runAllowFailure("settings delete system " + key);
        }
        return PrivilegedShell.runAllowFailure("settings put system " + key + " " + value);
    }
